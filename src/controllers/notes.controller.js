...
export const renderNotesqf = async (req, res) => {
  ...
  const ad = false;
  const ini = false;
  const usuario12 = await Usuario.findOne({email: req.user.email, t2:"S"});
...
export const renderNotescodigo2 = async (req, res) => {
  ...
  if (req.body.t2 == "a") {
    ...
    const doct1 = await Usuario.updateOne({email: req.user.email},  {t2:"S", p2:1, puntos:puntuacion});
  req.flash("success_msg", "Respuesta Correcta");
}
  else
  {
    req.flash("error_msg", "Respuesta Errada la Respuesta Correcta era la 'Prilartan'")

    await Usuario.findOneAndUpdate({email: req.user.email}, {t2:"S", p2:0}) 


};

  res.redirect("/notesqf");
...
export const renderNotes = async (req, res) => {
  ...
  const email = req.user.email;
    if (tip == "Jugador"){
      ...
      const ad = false;
      const ini = false;
      const trivia1 = await User.findOne({email: email, t1 :"S"});
      if (!trivia1) {
    ...
    //  console.log(notes1.email);
      if (!notes1) { 
        ...
        const puntos = 0;
         ...
         res.render("notes/all-notes-pronos", { notes, nombre, puntos, tp, ini, ad} );
     ...
     //             .rect(0, 0, doc.page.width, doc.header.options.heightNumber).fill("#ededed");
            ...
            doc2.setDocumentFooter({}, () => {
              ...
              //             .rect(0, doc.footer.y, doc.page.width, doc.footer.options.heightNumber).fill("#c2edbe");
                       ...
                       doc2.fill("#7416c8")
                           .fontSize(8)
                           .text("Quiniela Laboratorios Eticos", doc2.footer.x, doc2.footer.y + 10);
                   });
          // render tables
          doc2.render();
            // this should be the last
